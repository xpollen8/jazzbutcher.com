	...
	return <Suspense fallback=<>Loading...</> >
		{(!isLoading && releases) && <div className="listItem">
			...
			<div className="flex flex-wrap gap-3 justify-center">
				{releases
					.filter((item: ReleaseType) => (types) ? types.filter((x: ReleaseType) => item?.type?.split(',').includes(x)) : true)
					.map((item: ReleaseType, key: number) => {
						...
						return (<div className="drop-shadow-sm">
							...
							<div style={{ maxWidth: '250px' }} className="outline outline-slate-300 drop-shadow-sm"><Link key={key} href={item.href}><Image src={`https://jazzbutcher.com/${thumb}_250.jpg`} width={250} height={250} alt="cover" /></Link>
