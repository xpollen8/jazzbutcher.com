		...
		<div className="release">
			<div className="album_details">
				...
				{!!(useImage?.length && useThumb?.length) &&
					...
					<div className="smalltext">{(typeof label === 'string') ? <div style={{ width: '200px' }} className="grid grid-cols-1 -mt-5">{label?.split(',').map(expand).map((x: any, key: number) => <div className="break-keep" key={key}>{x}<br /></div>)}</div> : label} {media}</div>
					</div>
				}
